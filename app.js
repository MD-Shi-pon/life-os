...
window.setStudyView = (view) => {
    document.getElementById('studyViewLog').style.display = view === 'log' ? 'block' : 'none';
    document.getElementById('studyViewManage').style.display = view === 'manage' ? 'block' : 'none';
    document.getElementById('btnViewLog').classList.toggle('active', view === 'log');
    document.getElementById('btnViewManage').classList.toggle('active', view === 'manage');
};
...
window.refresh = () => {
    ...
    const dLogDB = JSON.parse(localStorage.getItem('s_dayLog')) || {}, subs = JSON.parse(localStorage.getItem('s_subs')) || ["English", "Marketing"];
